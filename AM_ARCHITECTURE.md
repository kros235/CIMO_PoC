...
> **문서 버전:** v0.17 (Day 8 전체 작업 완료 — 나머지 관찰 사항까지 원인 규명, 16.15.2번 항목)
> **최초 작성일:** 2026-03-24
> **최종 수정일:** 2026-07-06
> **상태:** POC Day 7 완료. Day 8 작업1~4 전체 완료
...
### 16.13 실시간·배치 요청 라인 분리 구현 (Day 8, 2026-07-05) — 16.11번, 16.12번 항목의 후속 조치
...
### 16.14 작업 3(RateLimitOperator 큐 개선 및 예약 정확도) 최종 결론 (Day 8, 2026-07-05)

작업 2까지 완료한 뒤, 16.7-1번 항목에서 예약 정확도 저하(83~85%)의 원인으로 지목했던 RateLimitOperator의 큐 자료구조 비효율을 개선하는 작업을 진행했다. 아래 여섯 가지를 순서대로 조치했으나, 예약 정확도는 개선 전후로 거의 변화가 없었다(83.6%~84.1%, 마지막 조치 후에는 오히려 79.2%로 하락).

**조치 내역과 결과:**

| 순번 | 조치 내용 | 조치 후 예약 정확도 |
|------|----------|---------------------|
| 1 | RateLimitOperator 대기열을 MapState에서 ValueState/ListState로 교체 (큐 길이에 비례하던 추가 비용을 일정한 비용으로 개선) | 84.0% |
| 2 | Flink TaskManager 슬롯 부족 문제 해결 (작업 1에서 늘어난 Job 개수에 맞춰 슬롯 증설) | 83.6~83.9% |
| 3 | 진단용으로 켜뒀다가 방치된 DEBUG 로그 레벨을 INFO로 원복 (대량 동시 로그 기록 자체가 지연 원인 중 하나였음을 확인) | 84.0% |
| 4 | RateLimitOperator의 병렬도를 채널 개수(5)와 동일하게 조정 (채널-일꾼 배정 불균형 발견) | 84.1% (배정은 여전히 불균등: 한 일꾼이 채널 3개를 담당) |
| 5 | RateLimitOperator의 maxParallelism을 5로 조정 (Key Group 개수를 일꾼 수와 맞춤) | 83.7% (배정은 여전히 불균등: MMS와 FAX가 같은 일꾼에 겹침 — Flink의 실제 해시 계산식을 파이썬으로 재현하여 이 현상이 우연이 아니라 이 채널 이름 조합에서 결정적으로 발생하는 겹침임을 확인) |
| 6 | maxParallelism을 15로 조정 (위 계산 재현 도구로 겹치지 않는 값을 찾아 적용) — Flink UI에서 채널 5개가 일꾼 5명에게 정확히 2,040건씩 균등하게 배정된 것을 확인 | **79.2%** (배정은 완벽하게 균등해졌으나 오히려 하락) |

**결론**: 6번 조치로 채널-일꾼 배정 불균형은 확실하게, 완전하게 해소했다(Flink UI 실측으로 검증 완료). 그럼에도 예약 정확도가 개선되지 않고 오히려 하락한 것은, 애초에 채널-일꾼 배정 불균형이 예약 정확도 저하(83~85%)의 지배적인 원인이 아니었다는 뜻이다. 서로 다른 성격의 원인 여섯 가지를 순서대로 개선했음에도 수치가 80~85% 범위를 벗어나지 못한 것은, 이번에도 이전 작업(작업 1, 작업 2)에서 확인했던 것과 같은 종류의 PoC 단일 호스트 환경의 구조적 한계일 가능성이 높다고 판단한다.
...
**제안하는 수정 방향 (사용자 확정 → 수정 완료)**:
1. STORE(성공) 시 UPDATE문에 `retry_count`, `channel` 컬럼 추가 → L01, L04 해결
2. DLQ(완전 실패) 판정 시에도 데이터베이스에 최종 상태(`status = 'DLQ'`)를 기록하는 UPDATE 신규 추가 → L02의 "영원히 고착" 문제(진짜 결함 부분) 해결
3. RETRY·FALLBACK 진행 중 상태는 그대로 DISPATCHING으로 유지하기로 확정 (별도 중간 상태 미도입)

#### 16.15.1 수정 구현 및 검증 결과 (2026-07-06)

**1차 수정**: `SendResultJob.java`의 STORE(성공) UPDATE문에 `retry_count`, `channel` 컬럼을 추가하고, DLQ(완전 실패) 판정 건에 대한 신규 UPDATE(`buildDlqPostgresSink()`)를 추가했다.

**1차 검증에서 추가로 발견된 문제**: `ts0003_retry_mechanism.py`로 검증한 결과, DLQ 상태 기록은 정상 동작했으나(`status='DLQ'` 5건 확인, `retry_count=0`도 정상 — 이 건들은 첫 시도부터 영구 실패라 재시도 자체를 거치지 않았기 때문), **재시도 횟수(`retry_count`)가 여전히 모든 건에서 0으로 남아있었다.** 원인을 다시 규명한 결과 두 가지가 추가로 확인됨:

- **`RetryJob.java`의 우회 경로**: 최대 재시도(3회) 초과로 완전 포기하는 경우, `RetryJob`이 `SendResultJob`을 거치지 않고 `topic.send.dlq`로 직접 발행하고 있어, 1차 수정한 데이터베이스 기록 로직 자체가 적용되지 않는 별도 경로였다.
- **`adapter_base.py`의 필드 위치 오류**: 발송 결과 응답을 조립하는 코드가 `payload.get("meta", {}).get("retryCount", 0)`로 "meta"라는 하위 항목 안에서 재시도 횟수를 찾고 있었으나, 실제 요청 메시지(`SendMessage`)는 `retryCount`를 최상위 항목에 바로 담아 보내 "meta" 항목 자체가 존재하지 않았다. 이 코드는 항상 못 찾고 기본값 0만 반환하고 있었다 — 이것이 "재시도하다가 결국 성공한 건도 재시도 횟수가 0으로 기록되는" 문제의 진짜 원인이었다.

**2차 수정**: `RetryJob.java`에 side output(`DLQ_TERMINAL_TAG`)을 추가해 완전 포기 건도 데이터베이스에 최종 상태를 기록하도록 하고, `adapter_base.py`의 필드 조회 위치를 `payload.get("retryCount", 0)`로 수정했다.

**최종 검증 결과** (`ts0003_retry_mechanism.py` 재실행): 재시도를 2회 거친 뒤 최종 성공(`DELIVERED`)한 건 4건에서 `retry_count = 2`가 정확히 기록됨을 확인했다.

```
tx_id                                | status    | channel | retry_count
17832685068380318700700000000000200 | DELIVERED | EMAIL   | 2
17832685068380318700700000000000163 | DELIVERED | SMS     | 2
17832685068370318700700000000000096 | DELIVERED | SMS     | 2
17832685068370318700700000000000098 | DELIVERED | SMS     | 2
```

**결론**: L01(재시도 횟수 미갱신), L02(DISPATCHING 상태 고착 — DLQ 종결 부분), L04(fallback 후 channel 미갱신) 세 가지 모두 코드 수정 및 실측 검증을 완료했다. L02의 "측정 타이밍 문제" 부분(재시도 진행 중인 건이 일시적으로 DISPATCHING으로 보이는 것)은 정상 동작이므로 별도 조치하지 않는다. L03(DISPATCHING 다중 해석)은 코드 결함이 아니므로 조치 대상에서 제외한다.

**아직 분석하지 않은 나머지 관찰 사항**: Mock Adapter healthcheck "unhealthy" 표시(→ 아래 16.15.2에서 조치 완료), 4xxxx 영구실패율 약 29%(설계 목표 약 2% 대비 높음, → 16.15.2에서 원인 규명), EMAIL 성공률 22%(FAX 90% 대비 낮음, → 16.15.2에서 원인 규명).

#### 16.15.2 나머지 관찰 사항 원인 규명 및 조치 완료 (2026-07-06)

**Mock Adapter healthcheck "unhealthy" 표시**: Adapter가 사용하는 기본 이미지(`python:3.11-slim`)에는 `curl`이 설치되어 있지 않은데, `docker-compose.adapters.yml`의 healthcheck 설정이 `curl -f http://localhost:PORT/health`로 되어 있었다. 컨테이너 안에서 매번 "명령어를 찾을 수 없음"으로 실패해 Docker가 이를 실제 서버 장애로 오해하고 있었다 — 밖에서(호스트에서) 포트로 직접 확인하면 정상 응답(200)이 나오던 이유. curl을 새로 설치하는 대신 파이썬에 내장된 `urllib`으로 healthcheck 명령을 교체해 조치 완료. 재기동 후 5개 Adapter 전부 `(healthy)`로 정상 표시됨을 확인했다.

**4xxxx 영구실패율 약 29%, EMAIL 성공률 22% — 원인 규명**: 각 Adapter의 결과코드 분포 로직(`main.py`)을 직접 계산한 결과, 채널별 설계된 영구실패율은 SMS 3%, MMS 5%, RCS 2%, FAX 9%, EMAIL 2%(가중평균 약 4.2%)로 확인되었다. 실행 중인 컨테이너의 환경변수(`SMS_SUCCESS_RATE` 등)도 전부 기본값 그대로임을 확인하여, 코드·설정 어느 쪽에도 결함이 없음을 확정했다.

이 상태에서 부하 없이 조용한 환경(500건, 다른 부하 테스트와 겹치지 않는 시점)에서 재측정한 결과:
...
전 채널이 설계 예상치와 거의 정확히 일치했다. **즉 "29%"·"22%"라는 예전 관측값은 코드 결함이 아니라, 측정 당시 시스템에 부하가 몰려있던 상태(NiFi 단일 인스턴스 CPU 포화, 16.5번·16.9번·16.13.2번 항목에서 이미 확정한 것과 동일한 현상)에서 발생한 측정 조건의 문제였던 것으로 결론 내린다.** 정상적으로 처리됐어야 할 요청이 부하로 인해 타임아웃되거나 거절되면서 "실패"로 잘못 집계되었을 가능성이 높다. 코드 수정 대상이 아니다.

**작업 4 최종 결론**: 알려진 한계 7개 항목(재시도 횟수 미기록, 발송 상태 고착, 발송 상태 다중 해석, 채널 정보 미기록, Adapter healthcheck 오표시, 영구실패율 이상 수치, EMAIL 성공률 이상 수치) 전부에 대해 원인을 규명했다. 이 중 4개(재시도 횟수, 발송 상태 고착의 진짜 결함 부분, 채널 정보, healthcheck)는 코드 수정 및 실측 검증까지 완료했고, 3개(발송 상태 다중 해석, 영구실패율, EMAIL 성공률)는 코드 결함이 아닌 것으로 확인되어 별도 조치가 불필요함을 확정했다. 이로써 Day 8의 4개 작업(요청 라인 분리, 대량 접수 실패 원인 규명, 예약 발송 대기열 개선, 기존 알려진 한계 분석) 전체가 완료되었다.

---

*문서 끝 — Day 8 전체 작업 완료 (v0.17)*
