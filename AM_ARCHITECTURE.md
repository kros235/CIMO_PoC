...
**Step 1. NiFi (수집·추적)**
- 트랜잭션 ID를 UUID v4로 생성하고 추적을 시작한다.
...
**트랜잭션 ID 생성 위치:** NiFi 수집 단계에서 최초 생성하며, 이후 전 구간(Kafka → Flink → Adapter → 결과 수신)에 걸쳐 동일 ID가 전파된다.
...
[발송 시스템]
   ...
   [배치 프로세스] ── 예약시각 도래 확인 + 건별 txId 생성(35자리)
      ↓
   [NiFi] ── 묶음 수집 → 건별 분리 → 추적 시작
      ↓
