...
> **문서 버전:** v0.6 (Day 7 예약 정확도 심층 진단 완료 반영)
> **최초 작성일:** 2026-03-24
> **최종 수정일:** 2026-07-05
> **상태:** POC Day 6 완료, Day 7 실시간·예약 발송 파트 완료 (배치 100만건 정식 측정·혼합 시나리오 진행 예정)
...
**최종 결론**: 채널별 TPS 제한(SMS 500 + MMS 200 + RCS 300 + FAX 100 + EMAIL 400 = 합계 1,500)은 각 채널이 이 속도를 **독립적으로** 낼 수 있다는 전제로 설계되었으나, 실제로는 5개 채널이 §16.5에서 이미 확인된 **동일한 단일 인스턴스 처리 한계(NiFi/Kafka, 약 500 TPS대)** 를 공유한다. 즉 채널별 계산상으로는 여유(1,500 TPS)가 있어 보여도, 합산 처리량은 이미 PoC 하드웨어 한계를 넘어서는 수준이라 각 채널의 타이머 방출이 이론치보다 항상 늦어진다. **이는 새로운 버그가 아니라 §16.5와 동일한 뿌리(PoC 단일 인스턴스 한계)에서 나온 또 다른 증상으로 판정한다.**

**PoC 범위에서의 처리 방침**: `RateLimitOperator` 카운트 리셋 버그는 정확성 개선 차원에서 수정 상태를 유지한다. "±60초 이내 99%" 기준은 실 서버(NiFi 클러스터 + Kafka 멀티 브로커) 스케일아웃을 전제로 한 목표였음을 명시하고, PoC 단일 인스턴스 환경에서는 **"발송 자체는 100% 정확히 완료되나(유실·중복 없음), 완료 시각 정밀도는 하드웨어 확장 후 재검증"** 으로 결론짓는다.

### 16.8 Day 7 남은 검증 항목 (배치·혼합 시나리오)

- ⏳ **시나리오 B (배치성 발송, TS-0008)**: 100만 건 정식 규모 배치 처리 시간 측정 (1만 건 축소검증은 완료, 예약 정확도는 §16.7 결론으로 종결)
- ⏳ **시나리오 C (복합 발송, TS-0009)**: 실시간+배치 동시 투입 시 상호 영향, Kafka 토픽 파티션 격리 효과

---

*문서 끝 — 다음 업데이트: TS-0008 100만건 정식 측정, TS-0009(혼합) 부하 테스트 완료 후 실측 수치 반영 예정 (v0.7)*
