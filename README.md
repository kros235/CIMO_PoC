...
| B-2 | 100만 건 배치 투입 → 처리 완료까지 소요 시간 측정 | 1만 건 축소검증 PASS (19~20초, 기준 900초 이내) → 100만 건 정식 측정 예정 | 🔄 |
| B-3 | 예약 시각 정확도 검증 (예약 시각 기준 ±60초 이내 발송 완료 확인) | 1만 건 기준 83.7~85.1% 관측 → **PoC 하드웨어 한계로 원인 확정**(§16.6.4), 기준 현실화 필요 | ✅ (원인 규명 완료, 목표 미달) |
| B-4 | Flink TaskManager 4개 구성에서 배치 처리량 재측정 | - | ⬜ 미실시 — 시나리오 A의 A-4/A-5와 동일하게 보류 |

**시나리오 B 완료 기준 — 재검토 결과:**
- [ ] 100만 건 배치: 전체 처리 완료 시간 측정 (목표: 15분 이내) → 1만 건 기준 19~20초로 선형 환산 시 100만 건도 여유 있게 달성 가능할 것으로 예상되나, 정식 측정 필요
- [x] ~~예약 정확도: 예약 시각 기준 ±60초 이내 발송 비율 99% 이상~~ → **실측 83~85% 수준으로 확정, 목표 미달.** §16.6.4 참고 — PoC 단일 인스턴스 하드웨어 한계로 판정(§16.5와 동일 계열), 실 서버 스케일아웃 전제 하에 재검증 필요. PoC 목적(구조 검증)상 이 수치를 억지로 맞추기보다 원인·한계를 규명하는 것으로 종결
...
*최종 업데이트: 2026-07-05 | 다음 작업: TS-0008 100만건 정식 측정, TS-0009(혼합 시나리오)*
