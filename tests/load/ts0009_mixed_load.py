...
설계 변경 이력 (중요 - README 원안과 다름):
    원안의 C-3은 "실시간 토픽 vs 배치 토픽 격리 효과"였으나, 실제 코드
    확인 결과 현재 구조는 발송방법코드와 무관하게 단일 요청 토픽
    (topic.send.request)을 공유하고, 채널별로만 분배된다
    (AM_ARCHITECTURE.md §16.11 참고 - 실시간/배치 요청 라인 완전 분리는
    향후 별도 과제로 이월됨). "실시간 토픽"과 "배치 토픽"이라는 구분
    자체가 없어 원안대로 테스트할 수 없다.
